...
class CustomerServiceOrchestrator:
    ...
    def __init__(self, build_index_on_start: bool = False):
        ...
        self.order_agent     = OrderAgent(guardrail=self.guardrail)
        self.logistics_agent = LogisticsAgent(retriever=self.retriever)
        self.refund_agent    = RefundAgent(retriever=self.retriever, guardrail=self.guardrail)
    ...
    def process_message(self, user_input: str, session_id: str = None) -> Dict:
        ...
        # ── Step 0: 提示词注入前置过滤（路由前，覆盖所有 Agent）──
        if self.guardrail._check_prompt_injection(user_input):
            self.stats["guard_blocks"] += 1
            block_msg = "检测到不合规的请求内容，无法处理。如需帮助请描述您的具体问题。"
            print(f"🛡️ [GuardRail] 提示词注入拦截，终止路由\n")
            session["messages"].append({
                "role": "assistant", "content": block_msg,
                "agent": "guardrail", "timestamp": datetime.now().isoformat(),
            })
            return {
                "success": False,
        ...
        biz_msg = Message(
            ...
            data={
                **router_resp.data,
                # 透传 session 历史给 ComplaintAgent 渐进式越狱检测使用
